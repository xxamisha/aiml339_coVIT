...
def run_seed_sweep(
    ...
    test_loader,
    device,
    ckpt_base_dir="./checkpoints/seed_sweep",  # local by default; pass a
                                                # /content/drive/... path
                                                # explicitly if running on Colab
    num_epochs=1,
    lr=1e-5,
    new_lr=None,  # LR for GPSA's own new params (pos_proj, gating_param); None = same as lr
    log_every=100,
    grad_accum_steps=1,
):
    """
    Args:
        ...
        ckpt_base_dir: each seed gets its own subfolder here — e.g.
            ...
            share a file.
        grad_accum_steps: if your train_loader's batch_size had to be
            reduced for VRAM reasons (e.g. 32 -> 16 on an 8GB GPU), set
            this to the ratio (e.g. 2) to recover the original EFFECTIVE
            batch size via gradient accumulation. Because this model uses
            LayerNorm (not BatchNorm), accumulating N micro-batches before
            stepping the optimizer gives gradients mathematically
            equivalent to one true batch of (micro_batch_size *
            grad_accum_steps) — not an approximation, an exact match up
            to floating-point rounding. Set to 1 (default) to disable.
    """
    ...
    for seed in seeds:
        ...
        # differential LR: GPSA's own new params (pos_proj, gating_param)
        # get a higher LR than the pretrained backbone, since they start
        # from scratch and need to move further/faster than weights that
        # are already pretrained and just need gentle nudging. Falls back
        # to a single LR for all params if the model has no such params
        # (e.g. the vanilla-phikon classifier).
        ...
        for epoch in range(start_epoch, num_epochs):
            for step, (x, y, metadata) in enumerate(train_loader):
                ...
                x, y = x.to(device), y.to(device)
                logits = model(x)
                # scale down so accumulated gradients match a single true
                # large-batch pass, not grad_accum_steps times larger
                loss = F.cross_entropy(logits, y) / grad_accum_steps
                loss.backward()
                final_loss = loss.item() * grad_accum_steps  # report unscaled loss
