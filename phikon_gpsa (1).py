...
def inject_gpsa(model, local_layers=10, locality_strength=1.0, gating_init=1.0, use_local_init=True):
    ...
    Args:
        ...
        gating_init: initial value for the gating logit (pre-sigmoid). Default
            1.0 -> sigmoid ~0.73 (positional-heavy, per the paper). Try 0.0
            -> sigmoid = 0.5 for a gentler, less disruptive start when
            fine-tuning a pretrained backbone for only a few epochs.
